...
content of functions in other files of the project. A TU is the
...
There is also papers using a profile-based approach rather than a whole-program
analysis for cross-module optimizations.  Xinliang \textit{et al.}
\cite{lipo} proposed LIPO, which embed IPA in the generated
...
phony objects dumps and link time wrappers when compared to LTO. Results showed
an speedup of up to $10\times$ in compilation time when compared to Feedback
...
into assembly by GCC. We managed speedups of up to a $2.4\times$ when
compiling individual files with 8 threads. Fig. \ref{fig:gcc_all_files} shows the results
...
\begin{figure}
\centering
	 \includegraphics[width=100mm,height=60mm]{figuras/times-insns-crop.pdf}
